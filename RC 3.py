    ...
    class base_game:
        ...
        def sposta(self, Event = None):
            ...
            if "j" in self.pulsanti and x12 > player_size/2 or "Left" in self.pulsanti and x12 > player_size/2:
                mappa.move(p2, - v_p2.movement_distance * self.resize, 0)
                x1, y1, x2, y2 = mappa.coords(p2)
            if "w" in self.pulsanti and "d" not in self.pulsanti and "a" not in self.pulsanti and "s" not in self.pulsanti:
                v_p1.gunpos = "n"
            elif "a" in self.pulsanti and "d" not in self.pulsanti and "w" not in self.pulsanti and "s" not in self.pulsanti:
                v_p1.gunpos = "o"
            elif "s" in self.pulsanti and "d" not in self.pulsanti and "a" not in self.pulsanti and "w" not in self.pulsanti:
                v_p1.gunpos = "s"
            elif "d" in self.pulsanti and "w" not in self.pulsanti and "a" not in self.pulsanti and "s" not in self.pulsanti:
                v_p1.gunpos = "e"
            if "i" in self.pulsanti and "l" not in self.pulsanti and "j" not in self.pulsanti and "k" not in self.pulsanti:
                v_p2.gunpos = "n"
            elif "j" in self.pulsanti and "l" not in self.pulsanti and "i" not in self.pulsanti and "k" not in self.pulsanti:
                v_p2.gunpos = "o"
            elif "k" in self.pulsanti and "l" not in self.pulsanti and "j" not in self.pulsanti and "i" not in self.pulsanti:
                v_p2.gunpos = "s"
            elif "l" in self.pulsanti and "i" not in self.pulsanti and "j" not in self.pulsanti and "k" not in self.pulsanti:
                v_p2.gunpos = "e"
            if "Up" in self.pulsanti and "Right" not in self.pulsanti and "Down" not in self.pulsanti and "Left" not in self.pulsanti:
                v_p2.gunpos = "n"
            elif "Left" in self.pulsanti and "Right" not in self.pulsanti and "Up" not in self.pulsanti and "Down" not in self.pulsanti:
                v_p2.gunpos = "o"
            elif "Down" in self.pulsanti and "Right" not in self.pulsanti and "Left" not in self.pulsanti and "Up" not in self.pulsanti:
                v_p2.gunpos = "s"
            elif "Right" in self.pulsanti and "Up" not in self.pulsanti and "Left" not in self.pulsanti and "Down" not in self.pulsanti:
                v_p2.gunpos = "e"
            v_p1.weapon_update()
            v_p2.weapon_update()
        ...
        def bullet_movement(self):
            for obj_id in mappa.find_withtag(self.tpid):
                if self.weapon != 1:
                    ...
                    tags = mappa.gettags(obj_id)
                    if (tags[0] == "n" and px1 <= x1 <= px2 and y1+50 >= py1 >= y1 and py1 < ppy1) or \
                        (tags[0] == "e" and py1 <= y1 <= py2 and x1-50 <= px1 <= x1 and px1 > ppx1) or \
                        (tags[0] == "s" and px1 <= x1 <= px2 and y1-50 <= py1 <= y1 and py1 > ppy1) or \
                        (tags[0] == "o" and py1 <= y1 <= py2 and x1+50 >= px1 >= x1 and px1 < ppx1):
                        mappa.delete(obj_id)
                        if self.pid == p1:
                            v_p2.damage_taken = float(tags[3])
                        elif self.pid == p2:
...
def start_menu(color1,color2,outline_color1,outline_color2,resize,movement_distance1,movement_distance2,health1,health2):
    ...
    build = tk.Label(main, text = "Build 7.15", fg="#bebebe", background="#ffffff")
