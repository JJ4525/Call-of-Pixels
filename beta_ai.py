...
def manual_console_control():
    ...
    if cmd == "move.right":
        ...
        p2_shoot()
    base.after(50, manual_console_control)
# -----controllo----- #
def tc(tag):
...
def calc_dx(x1,y1,x2,y2):
    dx = abs(x2 - x1)
    dy = abs(y2 - y1)
    distanza = math.sqrt(dx**2 + dy**2)
    return dx, dy, distanza
# -----ai shoot----- #
def ai_shoot():
    p2_shoot()
    xp1,yp1,xp2,yp2 = mappa.coords(p1)
    x1, y1, x2, y2 = mappa.coords(p2)
    if y1 <= yp1 <= y2 or y1 <= yp2 <= y2 or x1 <= xp1  <= x2 or x1 <= xp2 <= x2:
        base.after(10, ai_shoot)
# -----inizzializza ai----- #
def ai_mechanics():
    global gp2, weapon2, reflexes
    xp1,yp1,xp2,yp2 = mappa.coords(p1)
    x1, y1, x2, y2 = mappa.coords(p2)
    if y1 <= yp1 <= y2 or y1 <= yp2 <= y2 or x1 <= xp1  <= x2 or x1 <= xp2 <= x2:                            
        if xp2 < x1:
        ...
        base.after(reflexes, ai_shoot)             
    base.after(reflexes, ai_mechanics)        
# -----movimento----- #
def premuto(event):
