...
def a_star(grid, start, end, buffer_distance=10):
    ...
    while open_list:
        ...
        for next_position in neighbors:
            ...
            for obj in grid.find_overlapping(next_position[0] - buffer_distance, next_position[1] - buffer_distance, next_position[0] + 10 + buffer_distance, next_position[1] + 10 + buffer_distance):
                if "obstacle" in grid.gettags(obj):
...
class base_game:
    ...
    def nodes_plot(self, event = None):
        ...
        start_pos = self.get_start_end(p2,"start",True)
        end_pos = self.get_start_end(p1,"end",True)
    ...
    def a_star_start(self, start_pos,end_pos):
        ...
        for a in mappa.find_withtag("end"):
            mappa.itemconfig(a,tags = ("grid","empty"))
        self.get_start_end(p2,"start",False)
        self.get_start_end(p1,"end",False)
    def get_start_end(self, player,tag,ret):
