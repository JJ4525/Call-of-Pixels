...
def start_game(weapon1,weapon2,player_size, color1, color2, outline_color1, outline_color2, resize, movement_distance1, movement_distance2, health1, health2, mode):
    if mode != "versus":
        import heapq        
        def distance(p1, p2):
            x1, y1 = p1
        ...
        def a_star(grid, start, end):
            ...
            while open_list:
                ...
                for next_position in neighbors:
                    ...
                    next_node.h = distance(next_node.position, end_node.position)
    ...
    class base_game:
        ...
        if mode != "versus":
            ...
            def nodes_plot(self, event = None):
                start_pos = None
                end_pos = None
                ...
                start_pos = self.get_start_end(p2,"start",True,"empty")
                end_pos = self.get_start_end(p1,"end",True,"empty")
                self.a_star_start(start_pos,end_pos)
            def los(self):
                coords1 = mappa.coords(p1)
                coords2 = mappa.coords(p2)
                center_x2 = coords2[0] + player_size / 2
                center_y2 = coords2[1] + player_size / 2
                if coords1[0] <= center_x2 <= coords1[2]:
                    return False
                if coords1[1] <= center_y2 <= coords1[3]:
                    return False
                return True
            def a_star_start(self, start_pos,end_pos):
                ...
                for a in mappa.find_withtag("end"):
                    mappa.itemconfig(a,tags = ("grid","empty"))
                self.get_start_end(p2,"start",False,"empty")
                self.get_start_end(p1,"end",False,"empty")
            def get_start_end(self, player,tag,ret,tagch):
                x1,y1,x2,y2 = mappa.coords(player)
                for a in mappa.find_overlapping(x1+14,y1+14,x2-14,y2-14):
                    x,y,xx,yy = mappa.coords(a)
                    ...
                    if len(tags) > 1:
                        if tags[1] == tagch:
                            ...
                            if tagch == "empty":
                                self.end = pos
                            mappa.itemconfig(a,tags = ("grid",tag,"colored"))
                            if ret:
                                return pos
            def update_nodes(self,Event = None):
                ...
                for a in mappa.find_withtag("end"):
                    mappa.itemconfig(a,tags = ("grid","empty"))
                for a in mappa.find_withtag("path"):
                    mappa.itemconfig(a, tags=("grid", "empty"))
                start_pos = self.get_start_end(p2,"start",True,"empty")
                end_pos = self.get_start_end(p1,"end",True,"empty")
                self.a_star_start(start_pos,end_pos)
            ...
            def align(self):
                ...
                found = False
                while not found:
                    for a in mappa.find_overlapping(x1-radius,y1-radius,x2,y2):
                        tags = mappa.gettags(a)
                        if len(tags) > 1:
                            if tags[1] == "path":
                                obj = a
                                found = True
                    radius -=1
                obj_x0, obj_y0, obj_x1, obj_y1 = mappa.coords(p2)
                obj_center_x = (obj_x0 + obj_x1) / 2
                obj_center_y = (obj_y0 + obj_y1) / 2
                target_x0, target_y0, target_x1, target_y1 = mappa.coords(obj)
                target_center_x = (target_x0 + target_x1) / 2
                target_center_y = (target_y0 + target_y1) / 2
                dx = target_center_x - obj_center_x
                dy = target_center_y - obj_center_y
                mappa.move(p2, dx, dy)
            def shoot_ai(self):
                coords1 = mappa.coords(p1)
                coords2 = mappa.coords(p2)
                ...
                v_p2.gunpos = gp
                v_p2.weapon_update()
                if not "Return" in self.pulsanti:
                    self.pulsanti.append("Return")
            def move_auto(self,Event = None):
                if self.los():
                    if "Return" in self.pulsanti:
                        self.pulsanti.remove("Return")
                    if self.end != self.get_start_end(p1,"end",True,"end"):
                        self.update_nodes()
                    def try_up():
                        mappa.move(p2, 0, - 30)
                        x1,y1,x2,y2 = mappa.coords(p2)
                        for a in mappa.find_overlapping(x1,y1,x2,y2):
                            tags = mappa.gettags(a)
                            if len(tags) > 1:
                                if tags[1] == "path":
                                    mappa.move(p2, 0, 30)
                    ...
                    for a in mappa.find_overlapping(x1,y1,x2,y2):
                        ...
                        if len(tags) > 1:
                            if tags[1] == "path":
                                mappa.itemconfig(a, fill = "",tags=("grid", "empty"))
                    available = []
                    if try_up():
                        available.append("up")
                    if try_down():
        ...
        def shoot(self, event = None):
            if "space" in game.pulsanti:
                ...
                if v_p1.minigun_cooldown < -5:
                    ...
                    v_p1.minigun_cooldown -= 1
            if "Return" in game.pulsanti and mode != "pathfind":
...
def start_menu(color1, color2, outline_color1, outline_color2, resize, movement_distance1, movement_distance2, health1, health2):
    def close_window(mode, event = None):
        main.destroy()
        select_menu(color1, color2, outline_color1, outline_color2, resize, movement_distance1, movement_distance2, health1, health2, mode)
    def exit(event=None):
        main.destroy()
    root = tk.Tk()
    ...
    x1, y1 = width*(3/8), height*(5/8)
    x2, y2 = width*(5/8), height*(5/8) + 80
    bgcanv.create_rectangle(x1, y1, x2, y2, fill=color2,outline = "", tags="versus")
    bgcanv.create_text((x1 + x2) / 2, (y1 + y2) / 2, text="Player VS Player", fill="white", font=("Small Fonts", 30, "bold"), tags="versus")
    for item in bgcanv.find_withtag("versus"):
        bgcanv.tag_bind(item, '<ButtonPress-1>', lambda event: close_window("versus"))
    y1 += 100
    y2 += 100
    bgcanv.create_rectangle(x1, y1, x2, y2, fill=color1,outline = "", tags="computer")
    bgcanv.create_text((x1 + x2) / 2, (y1 + y2) / 2, text="Player VS Computer",fill="white", font=("Small Fonts", 30, "bold"), tags="computer")
    for item in bgcanv.find_withtag("computer"):
        bgcanv.tag_bind(item, '<ButtonPress-1>', lambda event: close_window("computer"))
    y1 += 100
    y2 += 100
    bgcanv.create_rectangle(x1, y1, x2, y2, fill=color2,outline = "", tags="pathfind")
    bgcanv.create_text((x1 + x2) / 2, (y1 + y2) / 2, text="Study Pathfinding",fill="white", font=("Small Fonts", 30, "bold"), tags="pathfind")
    for item in bgcanv.find_withtag("pathfind"):
        bgcanv.tag_bind(item, '<ButtonPress-1>', lambda event: close_window("pathfind"))
    titolo = tk.Label(main, text="CALL OF PIXELS", font=("Small Fonts", 120, "bold"), fg="#000000", background="#ffffff")
    titolo.place(x=width/2 - titolo.winfo_reqwidth()/2, y=100)
    build = tk.Label(main, text="Build 16.0", fg="#bebebe", background="#ffffff")
    ...
    p2_shoot() 
    main.focus_force()
    main.bind("<Return>",close_window)
    main.bind("<Escape>", exit)
    main.mainloop()
def select_menu(color1, color2, outline_color1, outline_color2, resize, movement_distance1, movement_distance2, health1, health2, mode):
    class Game:
        def __init__(self):
            self.color1 = color1
            self.color2 = color2
...
[[12, 7], [23, 8], '#666666'], [[12, 9], [23, 8], '#5B5B5B'], [[20, 6], [21, 9], '#333333'], [[17, 6], [18, 9], '#333333'], [[14, 6], [15, 9], '#333333'], [[9.5, 6.5], [11.5, 7], '#171717'], [[9.65, 7], [11.35, 12.35], '#63331f'], [[9.75, 7.25], [10.9, 7.75], '#FEEA3B'], [[10.9, 7.25], [11.25, 7.75], '#FEC006'], [[9.75, 8.0], [10.9, 8.5], '#FEEA3B'], [[10.9, 8.0], [11.25, 8.5], '#FEC006'], [[9.75, 8.75], [10.9, 9.25], '#FEEA3B'], [[10.9, 8.75], [11.25, 9.25], '#FEC006'], [[9.75, 9.5], [10.9, 10.0], '#FEEA3B'], [[10.9, 9.5], [11.25, 10.0], '#FEC006'], [[9.75, 10.25], [10.9, 10.75], '#FEEA3B'], [[10.9, 10.25], [11.25, 10.75], '#FEC006'], [[9.75, 11.0], [10.9, 11.5], '#FEEA3B'], [[10.9, 11.0], [11.25, 11.5], '#FEC006'], [[9.75, 11.75], [10.9, 12.25], '#FEEA3B'], [[10.9, 11.75], [11.25, 12.25], '#FEC006']], [[[10, 3], [20, 6], '#383838'], [[10, 6], [13, 7], '#383838'], [[11, 7], [12, 9], '#707070'], [[14, 6], [15, 8], '#292929'], [[13, 1], [14, 3], '#383838'], [[13, 1], [19, 2], '#383838'], [[19, 1], [18, 3], '#383838'], [[20, 4], [23, 5], '#2B2B2B']])
    ...
    def close_window(event = None):
        main.destroy()
        start_game(menu1.weapon,menu2.weapon,player_size, game.color1, game.color2, game.outline_color1, game.outline_color2, game.resize, game.movement_distance1, game.movement_distance2, game.health1, game.health2,mode)
    def exit(event = None):
        main.destroy()
    def open_settings():
    ...
    main.bind("<Return>",close_window)
    main.bind("<Escape>", exit)
    main.bind("<a>", menu1.move_left)
    main.bind("<d>",menu1.move_right)
    main.bind("<l>",menu2.move_right)
    main.bind("<j>",menu2.move_left)
    main.bind("<Right>",menu2.move_right)
    main.bind("<Left>",menu2.move_left)
    main.mainloop()
weapon1 = 1
weapon2 = 1
player_size = 10
